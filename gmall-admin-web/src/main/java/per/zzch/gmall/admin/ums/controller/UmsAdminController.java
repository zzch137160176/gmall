...
import per.zzch.gmall.vo.product.UmsAdminLoginParam;
import per.zzch.gmall.vo.product.UmsAdminParam;
import per.zzch.gmall.admin.utils.JwtTokenUtil;
...
public class UmsAdminController {
    ...
    public Object register(@Valid @RequestBody UmsAdminParam umsAdminParam, BindingResult result) {
        Admin admin = adminService.register(umsAdminParam);

        if (admin == null) {
            return new CommonResult().validateFailed("注册失败，用户名已存在");
        }
    ...
    public Object getAdminInfo(HttpServletRequest request) {
        ...
        // getOne方法由mybatis-plus自动生成，带了泛型
        // dubbo无法调用泛型
        // mybatis-plus中不要使用远程调用
